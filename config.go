...
type Config struct {
	Listen string `default:"0.0.0.0:8080" split_words:"true" desc:"Host and port to listen on"`
	...
	GroupHeader      string            `default:"" split_words:"true" desc:"HTTP header name that holds user group name to filter by, e.g. X-Ldap-Group. * in header value mean no group filter."`
}
